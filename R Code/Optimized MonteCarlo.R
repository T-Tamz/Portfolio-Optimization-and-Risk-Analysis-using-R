...
max_sharpe_idx <- which.max(multiple_Weight$sharpe_ratio)
optimal_portfolio <- multiple_Weight[max_sharpe_idx, ]


#Montecarlo Simulation with optimal portfolio
...
portfolio_return <- optimal_portfolio$expected_return /100
portfolio_volatility <- optimal_portfolio$volatility /100
...
mc_growth_plot <- ggplot(simulated_df_long, aes(x = day, y = portfolio_value, 
                                                group = simulation, 
                                                color = simulation)) +
  geom_line(alpha = 0.5) +
  theme_minimal() +
  labs(
    title = "Monte Carlo, Optimized Sharpe Ratio Portfolio Value Simulations",
    x = "Days",
    y = "Portfolio Value ($)"
  ) +
...
VaR_plot <- ggplot(final_values_df, aes(x = final_portfolio_values)) +
  ...
  geom_vline(xintercept = VaR_5, color = "red", linetype = "dashed", size = 1) +  
  theme_minimal() +
  labs(
